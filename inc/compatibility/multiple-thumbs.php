...
if (!class_exists('MetafansMultiPostThumbnails')) {
	...
	class MetafansMultiPostThumbnails {
		...
		public function register($args = array()) {
			...
			if (null === $this->label || null === $this->id) {
				if (WP_DEBUG) {
					trigger_error(sprintf(__("The 'label' and 'id' values of the 'args' parameter of '%s::%s()' are required", 'metafans'), __CLASS__, __FUNCTION__));
				}
				return;
			}
		...
		public function add_metabox() {
			add_meta_box("{$this->post_type}-{$this->id}", __($this->label, 'metafans'), array($this, 'thumbnail_meta_box'), $this->post_type, $this->context, $this->priority);
		}
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="MultiPostThumbnails.setAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">' . __( 'Set as %3$s', 'metafans' ) . '</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {
				add_thickbox();
				wp_enqueue_script( "metafans-mpt-featured-image", get_template_directory_uri() . '/assets/js/admin/multi-post-thumbnails/multi-post-thumbnails-admin.js' , array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media( array( 'post' => ( $post_ID ? $post_ID : null ) ) );
				wp_enqueue_script( "metafans-mpt-featured-image", get_template_directory_uri() . '/assets/js/admin/multi-post-thumbnails/multi-post-thumbnails-admin.js' , array( 'jquery', 'media-upload' ) );

				wp_enqueue_script( "metafans-mpt-featured-image-modal", get_template_directory_uri() . '/assets/js/admin/multi-post-thumbnails/media-modal.js', array( 'jquery', 'media-models' ) );
			}
		...
		public function admin_header_scripts() {
			...
			echo '<style>
				...
				.mf-multi-thumb .components-button.editor-post-featured-image__toggle {
					color: #444;
				}
				.mf-multi-thumb .components-button.editor-post-featured-image__toggle.has-cover {
					border: 0;
					padding: 0;
					background: transparent;
					height: auto;
				}
				.mf-multi-thumb .components-button.editor-post-featured-image__toggle img {
					max-width: 247px;
					width: 100%;
					height: 145px;
					position: relative;
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			$format_string = '<p class="mf-multi-thumb hide-if-no-js">' .
				'<button type="button" 
					title="%1$s" 
			...
			$set_thumbnail_link = sprintf(
				$format_string,
				sprintf( esc_attr__( "Set %s" , 'metafans' ), $this->label ),
				$this->post_type,
				$this->id,
				$url_class,
				$this->label,
			...
			$content = sprintf( $set_thumbnail_link, sprintf( esc_html__( "Set %s", 'metafans' ), $this->label ) );

			if ($thumbnail_id && get_post($thumbnail_id)) {
				$attr = array( 'class' => 'mf-thumbnail' );
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf(
						'<button type="button" class="hide-if-no-js components-button is-button is-default is-large is-secondary" onclick="jQuery(\'#set-%2$s-%3$s-thumbnail\').trigger(\'click\');return false;">%1$s</button>', __( 'Replace image', 'metafans' ), $this->post_type, $this->id );
					$format_string = '<button type="button" class="hide-if-no-js components-button is-link is-destructive" id="remove-%1$s-%2$s-thumbnail" onclick="MultiPostThumbnails.removeThumbnail(\'%2$s\', \'%1$s\', \'%4$s\');return false;">%3$s</button>';
					$content .= sprintf( $format_string, $this->post_type, $this->id, sprintf( esc_html__( "Remove %s", 'metafans' ), $this->label ), $ajax_nonce );
				}
			}

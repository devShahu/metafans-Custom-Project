var TophiveAutoCSS = window.TophiveAutoCSS || null;

( function( $, api ) {

    TophiveAutoCSS = function(){
        this.values = {};
        this.lastValues = {};
        this.devices = [ 'desktop', 'tablet', 'mobile' ];
    };
    TophiveAutoCSS._change = false;
    TophiveAutoCSS.prototype.fonts = {};
    TophiveAutoCSS.prototype.styling_fields = {
        color: null,
        image: null,
        position: null,
        cover: null,
    ...
    TophiveAutoCSS.prototype.subsets = {};
    TophiveAutoCSS.prototype.variants = {};
    TophiveAutoCSS.prototype.media_queries = {
        all: '%s',
        desktop: '%s',
    ...
    TophiveAutoCSS.prototype.box_shadow_fields = {
        color:  null,
        x:  0,
        y:  0,
        blur:  0,
    ...
    TophiveAutoCSS.prototype.reset = function(){
        this.fonts = {};
        this.subsets = {};
        this.variants = {};
        this.css = {
    ...
    TophiveAutoCSS.prototype.encodeValue =function( value ){
        return encodeURI( JSON.stringify( value ) )
    };
    TophiveAutoCSS.prototype.decodeValue = function( value ){
        return JSON.parse( decodeURI( value ) );
    };

    TophiveAutoCSS.prototype.loop_fields = function( fields, values, skip_if_val_null, no_selector ){
    ...
    TophiveAutoCSS.prototype.run = function( setting_name ){
        ...
        if ( window.Metafans_JS ) {
            if ( window.Metafans_JS.css_media_queries ) {
                this.media_queries = window.Metafans_JS.css_media_queries;
            }
        }
        this.reset();

        var that = this;
        that.loop_fields( Tophive_Preview_Config.fields );
        ...
        if ( $( '#tophive-google-font-css' ).length <= 0 ) {
            $( 'head' ).prepend( "<link rel='stylesheet' id='tophive-google-font-css'  href='' type='text/css' media='all' />" );
        }

        if ( url ) {
            //css_code = "\r\n@import url('"+url+"');\r\n\r\n"+css_code;
            $( '#tophive-google-font-css' ).attr( 'href', url );
        } else {
            $( '#tophive-google-font-css' ).remove();
        }

        css_code = css_code.trim();
        if ( $( '#tophive-style-inline-css' ).length <= 0 ) {
            $( 'head' ).append( "<style id='tophive-style-inline-css' type='text/css'></style>" )
        }
        $( '#tophive-style-inline-css' ).html( css_code );
    ...
    TophiveAutoCSS.prototype.get_setting = function(name, device, key  ){
        ...
        var value;
        var df = false;
        if ( !_.isUndefined( Tophive_Preview_Config.fields['setting|'+name ] ) ) {
            var field = Tophive_Preview_Config.fields['setting|'+name ];
    ...
    TophiveAutoCSS.prototype.get_google_fonts_url = function(){
    ...
    TophiveAutoCSS.prototype.join = function( object, glue ){
    ...
    TophiveAutoCSS.prototype.str_value = function( value, format, value_no_unit ){
        if ( _.isEmpty( value ) ) {
            return '';
        }
    ...
    TophiveAutoCSS.prototype.setup_color = function( value, format ){
        if ( format ) {
            if ( value ) {
    ...
    TophiveAutoCSS.prototype.setup_checkbox = function( value, format ){
        if ( format ) {
            if ( value ) {
                return format;
            }
    ...
    TophiveAutoCSS.prototype.setup_image = function( value, format ){
    ...
    TophiveAutoCSS.prototype.setup_slider = function ( value, format ){
        if ( ! _.isObject( value ) ) {
            value = {};
        }
    ...
    TophiveAutoCSS.prototype.setup_shadow = function ( value, format ){
        var that = this;
        if ( ! _.isObject( value ) ) {
            return '';
        }
    ...
    TophiveAutoCSS.prototype.setup_default = function( value, format ){
        if ( format ) {
            if ( value ) {
                return this.str_value( value, format );
            }
    ...
    TophiveAutoCSS.prototype.setup_css_ruler = function ( value, format ){
        if ( ! _.isObject( value ) ) {
            value = {};
        }
        value = _.defaults( value, {
    ...
    TophiveAutoCSS.prototype.setup_text_align = function( value, format ) {
        if ( format  ) {
            if ( value ) {
                return this.str_value( value, format );
            }
    ...
    TophiveAutoCSS.prototype.sanitize_color = function ( color ){
       return color;
    };

    TophiveAutoCSS.prototype.sanitize_slider = function ( value ){
        value = _.defaults( value, {
            unit: 'px',
            value: null
        });
    ...
    TophiveAutoCSS.prototype.sanitize_media = function ( value ) {
        if ( ! _.isObject( value ) ) {
            value = {};
        }
    ...
    TophiveAutoCSS.prototype.maybe_devices_setup = function( field, call_back, values, no_selector ) {
        var code = '';
        var code_array = {};
    ...
    TophiveAutoCSS.prototype.setup_font = function ( value ){
        if( ! _.isObject( value ) ) {
            value = {};
        }
        value = _.defaults( value, {
    ...
    TophiveAutoCSS.prototype.font = function( field, values ){
        var code = '';
        var that = this;
    ...
    TophiveAutoCSS.prototype.css_ruler = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_css_ruler', value, no_selector );
    };

    TophiveAutoCSS.prototype.shadow = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_shadow', value, no_selector );
    };

    TophiveAutoCSS.prototype.slider = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_slider', value, no_selector );
    };

    TophiveAutoCSS.prototype.color = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_color', value, no_selector );
    };

    TophiveAutoCSS.prototype.checkbox = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_checkbox', value, no_selector );
    };

    TophiveAutoCSS.prototype.image = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_image', value, no_selector );
    };

    TophiveAutoCSS.prototype.text_align = function( field, value, no_selector ){
        return this.maybe_devices_setup( field, 'setup_text_align', value, no_selector );
    };

    TophiveAutoCSS.prototype.setup_styling_fields = function( fields, list, selectors, type ){
        var that  = this;
        var newfs;
        var i;
        var newList = [];
    ...
    TophiveAutoCSS.prototype.styling = function( field ){
        var that = this;
        ...
        var listNormalFields = that.setup_styling_fields( normal_fields,  Tophive_Preview_Config.styling_config.normal_fields, selectors, 'normal' );
        var listHoverFields = that.setup_styling_fields( hover_fields,  Tophive_Preview_Config.styling_config.hover_fields, selectors, 'hover' );

        var listTabs = _.clone( Tophive_Preview_Config.styling_config.tabs );
        if ( tabs === false ) {
    ...
    TophiveAutoCSS.prototype.modal = function ( field ){
        var that = this;
        var values = this.get_setting( field.name, 'all' );
        if ( ! _.isObject( values ) ) {
            values = {};
    ...
    TophiveAutoCSS.prototype.setup_font_style = function ( value ){
        if ( ! _.isObject( value ) ) {
            value = {};
        }
    ...
    TophiveAutoCSS.prototype.html_replace = function( field, value ){
    ...
    TophiveAutoCSS.prototype.html_class = function( field, v ){
        var value;
        if ( _.isUndefined( v ) ) {
            value = _.clone( v );
        } else {
    ...
    TophiveAutoCSS.prototype.typography = function( field, values ){
        ...
        var fields = {};
        var devices_css = {};
        _.each( Tophive_Preview_Config.typo_fields, function( f ){
            fields[ f.name ] = f;
        } );
    ...
    var TophiveAutoCSSInit = new TophiveAutoCSS();

    api.bind( 'preview-ready', function() {
        //TophiveAutoCSSInit.run();
        TophiveAutoCSSInit.lastValues = api.get();
        TophiveAutoCSSInit.values = api.get();
    });

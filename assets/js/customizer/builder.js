(function($, wpcustomize) {
	...
	wpcustomize.bind("ready", function(e, b) {
		var addVersionChange = function(opts, builder, id, version) {
			if ( Tophive_Layout_Builder.hide_switcher === 'yes' ) {
				return ;
			}
			...
			if (typeof opts.versions === "object") {
				...
				if (select_options) {
					var $select_options = $(
						'<span class="tophive-swicth-version button-primary"><span>'+Tophive_Layout_Builder.swicth_version+'</span><select class="builder-version-change ">' +
							select_options +
							"</select></span>"
					);
					$(".tophive--cb-actions", builder.container).prepend(
						$select_options
					);
				}
				...
				$select_options.on("change", 'select', function() {
					var newVer = $(this).val();
					if ( tophivePanels[id + version] ) {
						tophivePanels[id + version].container.hide();
					}
					
					if (typeof tophivePanels[id + newVer] === "undefined") {
						var _builder;
						if ("v2" === newVer) {
						...
						tophivePanels[id + newVer] = _builder;
					}
					...
					if ( tophivePanels[id + newVer] ) {
						tophivePanels[id + newVer].container.show();
					}
		...
		_.each(Tophive_Layout_Builder.builders, function(opts, id) {
			var builder;
			var version = "v1";
			...
			tophivePanels[id + version] = builder;
				...
				if (
					...
					$(
						...
						'<p class="footer_moved_widgets_text">' +
							Tophive_Layout_Builder.footer_moved_widgets_text +
							"</p>"
					);
				}
			}, 1000);
		...
		wpcustomize.state("expandedSection").bind(function(section) {
			$(".tophive--device-panel .grid-stack-item").removeClass(
				"item-active"
			);
			$(".tophive--cb-row").removeClass("row-active");
			if (section) {
				$('.tophive--cb-row[data-id="' + section.id + '"]').addClass(
					"row-active"
				);
				$(
					".tophive--device-panel .grid-stack-item.for-s-" +
						section.id
		...
		function(e) {
			...
			if (template_name && template_name !== "") {
				...
				$.post(
					ajaxurl,
					{
						action: "tophive_builder_save_template",
						name: input.val(),
	...
	$document.on("mouseover", ".tophive--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
		var item = $(this);
		var nameW =
			$(".tophive--cb-item-remove", item).outerWidth() +
			$(".tophive--cb-item-setting", item).outerWidth();
	...
	$document.on("mouseleave", ".tophive--cb-row .grid-stack-item:not(.no-tooltip)", function(
		e
	) {
